...
grafos_desc <- foreach(i = unique_households, .packages= c(
  "tidyverse",
  "igraph",
  "haven",
  "tibble",
...
save(grafos_desc, file = paste0(getwd(), "/grafos_doparallel_descent.RData"))

plot(grafos_desc[300][[1]]$descent_net)
...
for(i in unique(data$household)) {
  ...
  V(marriage_net)$sex <- covariates$sex
...
# Usar foreach para ejecutar en paralelo
grafos_matri <- foreach(i = unique(data$household),
                        .packages = c(
                          "tidyverse",
                          "igraph",
                          "haven",
...
# Guardar los resultados en un archivo
save(grafos_matri, file = paste0(getwd(), "/grafos_doparallel_marriage.RData"))

plot(grafos_matri[300][[1]]$marriage_net)
...
for(i in unique(data$household)) {
  ...
  covariates <- covariates[order(covariates$id_persona),]
  V(dependency_net)$sex <- covariates$sex
  V(dependency_net)$edad <- covariates$edad
...
household_process <- function(i, data) {
  ...
  V(dependency_net)$sex <- as.character(covariates$sex)
...
grafos_dep <- foreach(i = unique_households,
                      #  .verbose =TRUE,
                      .packages = c(
                        "tidyverse",
                        "igraph",
...
# Guardar los resultados en un archivo
save(grafos_dep, file = paste0(getwd(), "/grafos_doparallel_dependency.RData"))

plot(grafos[300][[1]]$dependency_net)
...
for(i in unique(data$household)) {
  ...
  covariates <- covariates[order(covariates$id_persona),]
  V(kinship_net)$sex <- covariates$sex
  V(kinship_net)$edad <- covariates$edad
  
  
  grafo <- list(i = i, kinship_net = kinship_net)
  grafos[[length(grafos) + 1]] <- grafo
...
names(grafos) <- c(as.character(unique(data$household)))

plot(grafos_kin[200][[1]]$kinship_net)

#Do parallel Red Kinship
# Establecer el número de núcleos para el procesamiento en paralelo
num_cores <- 4
registerDoParallel(cores = num_cores)
...
# Obtener las households únicas
unique_households <- unique(data$household)

# Usar foreach para ejecutar en paralelo
grafos_kin <- foreach(i = unique_households, .packages = c(
  "tidyverse",
  "igraph",
  "tibble",
  "reshape2",
...
# Detener el registro de núcleos en paralelo
stopImplicitCluster()

# Guardar los resultados en un archivo
save(grafos_kin, file = "grafos_doparallel_kin.RData")
