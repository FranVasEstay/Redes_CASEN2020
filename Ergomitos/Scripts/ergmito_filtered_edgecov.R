...
for (i in seq_along(dependency_network_filtred)) {
  ...
  kinship_network <- kinship_network_filtred[[i]][['kinship_net']]
  ...
  if (all(variables %in% list.vertex.attributes(household_network))) {
    ...
    if (no_missing && has_variation &&
        ...
        #length(kinship_network[["val"]]) == 5 && # Size: exactly 3-5 nodes
      ...
      #filtered_kinship[[length(filtered_kinship) + 1]] <- kinship_network
